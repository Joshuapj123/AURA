...
function AgentNode({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative px-4 py-3 rounded-lg border min-w-[140px] text-center transition-all duration-300",
        border[status]
      )}
    >
      <Handle type="target" position={Position.Top} />
      <div className={cn("absolute top-2 right-2 w-2 h-2 rounded-full", dot[status])} />
      <div className="text-xl mb-0.5">{icon}</div>
      <div className="font-mono text-xs font-bold text-zinc-100">{label}</div>
...
type StageCfg = {
  ...
  edges: Record<string, { c: string; a: boolean }>;
};

const STAGES: Record<PipelineStage, StageCfg> = {
  idle: {
    nodes: { "1":"idle","2":"idle","3":"idle","4":"idle","5":"idle","6":"idle" },
    edges: {},
  },
  qa: {
    nodes: { "1":"success","2":"running","3":"idle","4":"idle","5":"idle","6":"idle" },
    edges: { "e1-2":{c:"#22d3ee",a:true} },
  },
  verify: {
    nodes: { "1":"success","2":"success","3":"running","4":"running","5":"running","6":"idle" },
    edges: { "e1-2":{c:"#34d399",a:false},"e2-3":{c:"#22d3ee",a:true},"e2-4":{c:"#22d3ee",a:true},"e2-5":{c:"#22d3ee",a:true} },
  },
  decision: {
    nodes: { "1":"success","2":"success","3":"success","4":"success","5":"success","6":"running" },
    edges: { "e2-3":{c:"#34d399",a:false},"e2-4":{c:"#34d399",a:false},"e2-5":{c:"#34d399",a:false},"e3-6":{c:"#22d3ee",a:true},"e4-6":{c:"#22d3ee",a:true},"e5-6":{c:"#22d3ee",a:true} },
  },
  done: {
    nodes: { "1":"success","2":"success","3":"success","4":"success","5":"success","6":"success" },
    edges: { "e3-6":{c:"#34d399",a:false},"e4-6":{c:"#34d399",a:false},"e5-6":{c:"#34d399",a:false} },
  },
  error: {
    nodes: { "1":"success","2":"success","3":"success","4":"error","5":"success","6":"error" },
    edges: { "e2-4":{c:"#ef4444",a:false},"e4-6":{c:"#ef4444",a:true} },
  },
};
...
export function AgentGraph({ stage }: { stage: PipelineStage }) {
  ...
  useEffect(() => {
    ...
    setNodes(ns =>
      ns.map(n => ({
        ...n,
        data: { ...n.data, status: cfg.nodes[n.id] ?? "idle" },
      }))
    );
    setEdges(es =>
      es.map(e => ({
